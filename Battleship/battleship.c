...
void vSetFleet(t_Board *Player)
{
   ...
   for (int i = 0; i < TOTALSHIPS; i++)
   {
      ...
      {
         //Platziere zuerst alle schiffe größer als 1
...
void vSetLongShip(t_Board *Player, int index) {
   ...
   do
   {
      ...
      if ((iYfirst == iYlast) || (iXfirst == iXlast)) {
         ...
         // Überprüfe den Input auf die  Schiffgröße
...
void vSetShortShip(t_Board *Player, int index)
{
   ...
   do {
      ...
      //Wir überprüfen ob die Koordinaten gültig sind, um ein Einser schiff platzieren zu können
      ...
      {
          //und verwalten alle benötigten Parameter
...
void vPlaceShip(t_Board *Player, int iXfirst, int iYfirst, int iXlast, int iYlast, int index) {
   int x = 0;
   int y = 0;
   for (int i = 0; i < Player->fleet[index].iLength; i++) { //wir gehen die gesamte länge des Schiffes durch
      if (iYfirst == iYlast) {
         // Horizontales Schiff
      ...
      Player->fleet[index].coordinates[i].iColumn = x; //und übergeben dem Schiff die Koordinaten
      ...
      Player->iTotalHits++; //und erhöhe die Total Player Hitpoints um die länge des schiffes
   }
}
...
int iShoot(t_Board *Enemy)
{
   ...
   printf("Please enter the coordinate that you want to shoot at\n"); //die Koordinate auf die wir schießen möchten
   int iX = iGetX("X Coordinate: ");
   int iY = iGetY("Y Coordinate: ");



   int iTemp = iCheckShot(iX, iY, Enemy->fleet, &iIndexOfShip); //Überprüfen ob wir etwas treffen oder nicht
   int iGoAgain = 0;
   ...
   case 1: //case wenn wir treffen
      ...
      iGoAgain = 1;
      break;
   case 2: //case wenn wir etwas abschießen, was wir schon getroffen haben
      system("cls");
